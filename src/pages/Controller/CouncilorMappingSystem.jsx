...
const CouncilorMappingSystem = () => {
  ...
  const handleMapping = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_local_host}/auth/assign-councilor`,
        {
          selectedCouncilorId: selectedLeader,
          assignCouncilorIds: selectedStudents,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      toast.success("Councilor mapped to councilor.");
      setSelectedStudents([]);
      setSelectedLeader(null);
    } catch (error) {
      console.error("Error mapping councilors:", error);
  
      // Check if the response indicates already mapped councilors
      if (error.response && error.response.status === 400 && error.response.data.message === "All selected councilors are already mapped") {
        toast.info("Selected councilors are already mapped.");
      } else {
        toast.error("An error occurred while mapping councilors.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <Card>
        <CardHeader className="mb-8 md:p-2 p-3 bg-[#01121F]">
          <div className="flex justify-between items-center">
            <Typography variant="h6" color="white">
              Councilor Mapping to Councilor
            </Typography>
        ...
        <div className="flex justify-between items-center py-3 px-5">
          <div className="md:w-72">
            <Select label="Mapping with councilor" onChange={(e) => setSelectedLeader(e)}>
              {councilors.map((item) => (
...
export default CouncilorMappingSystem;
