...
const SeniorForActiveStudent = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        // Ensure valid structure for activeStudents
        if (res.data.data && Array.isArray(res.data.data.activeStudents)) {
          setActiveStudents(res.data.data.activeStudents.filter(item => item)); // Filter out null or undefined items
        } else {
          console.error("Invalid data structure for active students");
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        setTeamLeaders(res.data.data || []); // Default to empty array if no data
      })
      .catch((error) => {
            ...
            <tbody>
              {activeStudents.length > 0 ? (
                activeStudents.map((item) => (
                  // Ensure 'item' is valid and has a 'name' property
                  item ? (
                    <tr key={item._id}>
                      <td className="py-3 px-5">
                        <div className="flex items-center gap-4">
                          <div>
                            ...
                            >
                              {item.name || "No name"} {/* Fallback for name */}
                            </Typography>
                            <Typography className="text-xs font-normal text-blue-gray-500">
                              {item.email || "No email"} {/* Fallback for email */}
                            </Typography>
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-5">
                        <Typography className="text-xs font-semibold text-blue-gray-600">
                          {item.role || "No role"} {/* Fallback for role */}
                        </Typography>
                      </td>
                      <td className="py-3 px-5">
                      ...
                      <td className="py-3 px-5">
                        <Typography className="text-xs font-semibold text-blue-gray-600 capitalize">
                          {item.refer_code || "No refer code"} {/* Fallback for refer code */}
                        </Typography>
                      </td>
                      <td className="py-3 px-5">
                        <Typography className="text-xs font-semibold text-blue-gray-600 capitalize">
                          {item.whatsappNo || "No number"} {/* Fallback for whatsappNo */}
                        </Typography>
                      </td>
